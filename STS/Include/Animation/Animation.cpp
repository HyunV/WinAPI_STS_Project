...
{
	if (FindInfo(SequenceName)) //시퀀스 이름을 탐색
		return;

	CAnimationSequence* Sequence = nullptr; 

	if (m_Scene) //씬이 있는지 확인
	{
	...
	Info->m_FrameTime = Info->m_PlayTime / Sequence->GetFrameCount(); //움짤 재생 시간 / 움짤 프레임 개수
	//1초에 3모션이면 m_FrameTime은 0.33초
...
{
	if (m_CurrentAnimation->m_Sequence->GetName() == Name) //같은 모션으로 바꾸려하면 리턴
		return;
	...
	//새걸로 교체되어야 하는 애니메이션이면 교체해주고 한번 더
	m_CurrentAnimation = Info;
...
void CAnimation::Update(float DeltaTime)
{
	...
	for (size_t i = 0; i < Size; ++i)
	{
		...
		{
			m_CurrentAnimation->m_vecNotify[i]->Call = true; //노티파이 호출 했으니 true
	...
	if (AnimationEnd)
	{
		...
		for (size_t i = 0; i < Size; ++i)
		{
			m_CurrentAnimation->m_vecNotify[i]->Call = false; //애니메이션 끝나면 중간에 true 했던 Notify를 다시 false로 바꿔준다.
		}
	}
}
