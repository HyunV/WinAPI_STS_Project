...
public:
	...
	void SetText(const TCHAR* Text)
	{
		...
		if (Length + 1 > m_Capacity) //배열 공간(널문자 +1)보다 클 경우
		{
			m_Capacity = Length + 1; //재할당해줌
		...
		lstrcpy(m_Text, Text); //인자 복사

		m_Count = lstrlen(m_Text); //길이 가지고 있게
	}
	void AddText(const TCHAR Text) //내가 원하는 문자열 추가
	{
