...
bool CTileMap::CreateTile(int CountX, int CountY, const Vector2& TileSize) //타일 쪼개기
{
    m_CountX = CountX;
    m_CountY = CountY;
    m_TileSize = TileSize;
    ...
    m_Scene->GetCamera()->SetWorldResolution(m_Size); //이걸로 월드 레졸루션 잡아준다.

    for (int i = 0; i < m_CountY; ++i)
    {
        for (int j = 0; j < m_CountX; ++j) //안쪽이 가로
        {
            CTile* Tile = new CTile; //타일 저장

            Tile->m_Scene = m_Scene; //타일이 씬을 가지게
            Tile->m_Owner = this;

            Vector2	Pos = m_TileSize * Vector2((float)j, (float)i); //이걸로 전체타일기준으로 위치를 잡아주고
